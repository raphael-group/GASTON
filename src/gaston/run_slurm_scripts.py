...
def train_NN_parallel(path_to_coords, path_to_glmpca, hidden_spatial, hidden_expression, 
                      ...
                      epochs=10000, checkpoint=500, optimizer='adam', num_seeds=30
                     ):
    ...
    for seed in range(num_seeds):
        ...
        create_job_script(seed, f"{output_dir}/seed{seed}", tasks, processes, time, mem_per_cpu, cmd, conda_environment,path_to_conda_folder)

def create_job_script(name, outDir, tasks, cpuPerTask, time, mem_per_cpu, command, environment,path_to_conda_folder):
