...
def pw_linear_fit(counts_mat, belayer_labels, belayer_depth, cell_type_df, ct_list,
                  umi_threshold=500, idx_kept=None, pc=1, pc_exposure=True, t=0.1,
                  depth_mult_factor=1, reg=0, zero_fit_threshold=0):
    ...
    nonzero_per_layer = np.zeros((G,L))
    for l in range(L):
        cmat_l=cmat[:,belayer_labels==l]
        nonzero_per_layer[:,l]=np.count_nonzero(cmat_l,1)

    inds1= ((pv_mat < t) & (nonzero_per_layer >= zero_fit_threshold))
    inds0= ((pv_mat >= t) | (nonzero_per_layer < zero_fit_threshold))
